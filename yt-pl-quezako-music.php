...
// SQLite.
try {
    $pdo = new PDO('sqlite:' . dirname(__FILE__) . '/db-quezako-music.sqlite');
...
if ($client->getAccessToken()) {
    ...
    usort($results, function ($a, $b) {
        ...
        $arrHtmlImages[-1] = "Watch Later<br /><input type='text' id='' name='' value=''
    	style='background: url(https://s.ytimg.com/yts/img/no_thumbnail-vfl4t3-4R.jpg);background-size: 100%;'
    	onDrop='convert(event);' onPaste='convert(event);' /><br />";
        
        foreach ($playlistsResponse['items'] as $playlist) {
            $tmp = "{$playlist['snippet']['title']}<br /><input type='text' id='{$playlist['id']}' name='{$playlist['id']}' value=''
        	style='background: url({$playlist['snippet']['thumbnails']['medium']['url']});background-size: 100%;'
        	onDrop='convert(event);' onPaste='convert(event);' /><br />";
