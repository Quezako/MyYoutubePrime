...
$stmt->execute(array('type' => 'token'));
$result = $stmt->fetch();
$DbToken = isset($result['token']) ? $result['token'] : null;

$stmt->execute(array('type' => 'refreshtoken'));
$result = $stmt->fetch();
...
if (isset($_GET['code']) && !isset($DbToken)) {
	...
	$stmt->execute(array(':token'=>$DbToken, ':type'=>'token'));
...
if ($client->getAccessToken()) {
	...
	foreach ($playlistsResponse['items'] as $playlist) {
		$htmlOptions .= "<option value='{$playlist['id']}'>{$playlist['snippet']['title']}</option>";
	...
	$htmlSelect = "<select id='playlist_id' name='playlist_id' style='width: auto;'><option value=''>Watch Later</option>$htmlOptions</select>";
		...
		try {
			if ($playlist_id != '') {
				...
				foreach ($channelsResponse['items'] as $channel) {
					...
					// uploaded to the channel, and then call the playlistItems.list method to retrieve that list.
			...
			$htmlBody .= sprintf('<p style="text-align: left;">%s (%s)</p>',
	...
	if (isset(json_decode($client->getAccessToken())->refresh_token)) {
		...
		$stmt = $pdo->prepare($sql);
		$stmt->execute(array(':token'=>$DbRefreshToken, ':type'=>'refreshtoken'));
...
function convert() {
	setTimeout(function () {
		var val = decodeURIComponent(document.getElementById('video_url').value);
		...
		if (typeof vid === "undefined") {
			...
			document.getElementById('video_ids').value = vid;
			form1.submit();
		}
    }, 500);
}
...
<form id="form1" action="$redirect" method="get" target="_self" style="height: 100%;">
	<!--
	Add URL to Watch Later
	<br /><br />
	-->
	$htmlSelect<br />
	<br />
	...
	<!--<br />-->
	<input type="text" id="video_url" name="video_url" value="" style="min-height: 700px;height: 90%;display:inline-block;position:relative;" onDrop="convert();" /> 
	<input type="text" id="video_ids" name="video_ids" value="" style="display: none;" /> 
	<!--
	<br /><br />
